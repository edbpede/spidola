...
The project is licensed **AGPL-3.0-or-later** (decided; SPDX identifier `AGPL-3.0-or-later` on every file, REUSE conventions throughout the repository). Three practical consequences are accepted with eyes open. First, **App Store friction**: Apple's standard App Store terms have historically been read (by the FSF, and in the VLC/GPLv2 episode) as conflicting with strong copyleft. As the sole initial copyright holder, the project owner can lawfully distribute their own AGPL code on the App Store — one cannot infringe one's own copyright — but the moment third-party AGPL contributions are merged, contributors' licenses bind the distribution too. The project's answer (**decided**) is a DCO-plus-additional-permission model: every contribution is signed off under the Developer Certificate of Origin and licensed AGPL-3.0-or-later together with the App Store Distribution Exception (`APPSTORE_EXCEPTION.md`), an additional permission under AGPL section 7 that keeps App Store distribution lawful once the code has multiple copyright holders. There is no CLA and no copyright assignment; the contributor-facing terms live in `CONTRIBUTING.md`. Second, **the network clause**: AGPL section 13 concerns users interacting with the software over a network; the pairing server (§6.1) must surface a "source code" link in its served pages to keep compliance trivially true. Third, **dependency compatibility**: all bundled components must be AGPL-compatible; in particular the mpv/FFmpeg builds embedded on both platforms are configured LGPL (no GPL-only FFmpeg components), which is both AGPL-compatible and the safer posture for App Store review. Within LGPL, the effective version is **LGPL-3.0 on both platforms (decided)**: every TLS provider FFmpeg can realistically use is either Apache-2.0 (Mbed-TLS on Android, OpenSSL 3 on tvOS) — which FFmpeg accepts only under `--enable-version3` — or GnuTLS, whose GMP dependency is LGPL-3.0 itself, so the only LGPL-2.1 FFmpeg is one that cannot open an `https://` stream (mpv itself stays LGPL-2.1-or-later; the combined stack's floor is v3). LGPL-3.0 is fully AGPL-3.0-compatible and a non-issue on Play. On the App Store it sharpens the friction already accepted above — LGPLv3's installation-information terms (GPLv3 §6, via LGPLv3 §4e) are argued by some to conflict with App Store distribution — and unlike the project's own code, no section-7 exception can be granted on FFmpeg's or GMP's authors' behalf. The risk is accepted with the same eyes open: on the mainstream reading, §6's installation-information duty attaches to transactions that transfer a *device*, which an app download is not; the complete corresponding source of both the app and the media stack, with pinned sources and build flags (`tools/build-libmpv-android/`, `tools/build-mpvkit/`), is public, so anyone can rebuild the combined work — modified libraries included — and install it on their own Apple TV, which is the re-link freedom the LGPL exists to protect; and the worst case falls back to the same answer as the store-policy worst case below, source-build distribution on tvOS while appealing. Engineering enforcement (license audits in CI) is specified in the tech spec.
